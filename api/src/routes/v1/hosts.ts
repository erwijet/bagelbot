...
hostsRouter.post("/", async (req, res) => {
  ...
  if (!userRecord)
    return res.status(400).json({ resp: "rejected", justification: "invalid identity tok" });
  ...
  try {
    ...
    return res
      .status(400)
      .json({ resp: "rejected", justification: "no authority with provided credentials" });
  }

  // step 2. verify min version and unique host requirement

  if (semverCompare(min_host_ver, ver) > 0)
    return res
      .status(400)
      .json({ resp: "rejected", justification: "supplied version is out of compliance" });

  const hostRecord = await HostModel.findOne({ host: loc });
  if (!!hostRecord && hostRecord.owner_id != userRecord._id.toString())
    return res
      .status(400)
      .json({
        resp: "rejected",
        justification:
          "requested host location " + loc + " has already been claimed by another user",
      });

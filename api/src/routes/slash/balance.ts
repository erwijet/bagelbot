...
balanceRouter.post("/", async (req, res) => {
  ...
  if (balance < 0)
    return res.end(
      "Hang on there! Look's like you have a brand new bryxcoin wallet / address. Make sure to reach out to a bagelbot admin about your bryxcoin buyin. You can also just host a bagel tab `/tab open` if you don't want to buy in"
    );

  return res.end(
    `BryxCoin balance for <@${user!.slack_user_id}> (100 BRYX = $1): \`${balance} BRYX\``
  );
});
